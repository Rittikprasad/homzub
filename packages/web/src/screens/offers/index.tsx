...
import OffersDropdown, {
  OffersDropdownType,
} from "@homzhub/web/src/screens/offers/components/OffersDropDown";
...
import {
  MadeSort,
  offerMadeSortBy,
} from "@homzhub/common/src/constants/Offers";
...
const Offers: FC<IProps> = (props: IProps) => {
  ...
  const getInitialOfferType = (): OfferType => {
    ...
    if (state.offerType)
      // @ts-ignore
      return state.isReceivedFlow
        ? OfferType.OFFER_RECEIVED
        : OfferType.OFFER_MADE;
  ...
  const title =
    offerType === "Offer Received"
      ? t("offers:noOfferReceived")
      : t("offers:noOfferMade");
  useEffect(() => {
    getOfferDetails();
    getOfferFilters();
  }, [offerType, close]);
  ...
  const getOfferDetails = async (): Promise<void> => {
    try {
      ...
      const isOfferReceivedInfoRead: boolean =
        (await StorageService.get(StorageKeys.IS_OFFER_RECEIVED_INFO_READ)) ||
        false;
      const isOfferMadeInfoRead: boolean =
        (await StorageService.get(StorageKeys.IS_OFFER_MADE_INFO_READ)) ||
        false;
    ...
    try {
      const response: OfferFilter = await OffersRepository.getOfferFilters(
        offerType === OfferType.OFFER_RECEIVED
          ? OfferFilterType.RECEIVED
          : OfferFilterType.CREATED
      );
      console.log("this is response 77777777777777777777777", response);
  ...
  const getPropertyListData = async (): Promise<void> => {
    ...
    const dynamicFilters = {
      ...(selectedFilters.countary_id && {
        countary_id: selectedFilters.countary_id,
      }),
      ...
      ...(selectedFilters.filter_by && {
        filter_by: selectedFilters.filter_by,
      }),
    ...
    const payload = {
      type:
        offerType === OfferType.OFFER_RECEIVED
          ? NegotiationOfferType.RECEIVED
          : NegotiationOfferType.CREATED,
      params: {
        ...dynamicFilters,
      },
    };
  ...
  const isInfoRead =
    offerType === OfferType.OFFER_RECEIVED
      ? offerReceivedInfoRead
      : offerMadeInfoRead;
  const infoText =
    offerType === OfferType.OFFER_RECEIVED
      ? t("offers:offerInfo")
      : t("offers:offerMadeInfo");
  if (!offerCountData) {
    return null;
  }

  const viewOffers = (
    payload: ICurrentOffer | null,
    assetId: number,
    ...
    if (offerType === OfferType.OFFER_MADE) {
      ...
      NavigationService.navigate(history, {
        path: RouteNames.protectedRoutes.OFFERS_LISTED_PROPERTY.replace(
          ":listingId",
          `${assetId}`
        ),
        params: {
          ...
          <View style={styles.infoSubContainer}>
            <View
              style={[
                ...
                (isTablet || isMobile) && styles.iconTextContainerMobile,
              ]}
            >
              <Icon
                name={icons.circularFilledInfo}
                size={15}
