...
const DropDown = ({
  ...
  const onLayout = (e: LayoutChangeEvent): void => {
    setWidth(
      e.nativeEvent.layout.width > 200 ? e.nativeEvent.layout.width : 200
    );
  };
  ...
  return (
    <Popover
      ...
      content={
        <PopupMenuOptions options={data} onMenuOptionPress={valueChange} />
      }
      popupProps={defaultDropDownProps(width)}
    >
      <View onLayout={onLayout}>
        {PlatformUtils.isWeb() ? content(popupRef) : content}
      </View>
    </Popover>
  );
};
