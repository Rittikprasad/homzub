...
class DateUtils {
  ...
  public getCurrentWeekStartDate = (
    format?: string,
  ...
  public getMonthRange = (startIndex: number, endIndex: number): string[] => {
    ...
    return [
      ...MonthNames.slice(startIndex),
      ...MonthNames.slice(0, endIndex + 1),
    ];
  };
  ...
  public getDateDifferenceMessage = (lastDate: string): string => {
    ...
    const millisecondDifference = Math.abs(
    ...
    if (isMoreThanAHour) {
      return I18nService.t("assetMore:hrAgo", {
        hour: Math.trunc(hoursDifference),
      });
    }
    if (isLessThanAWeek) {
  ...
  public convertDateFormatted = (
    date: string,
    format = DateFormats.DDMMMYYYY_Hm
  ): string => {
    return moment(date).format(format);
  };
  ...
  public ascendingDateSort = (
    ...
    [...arr].sort(
      (a, b) =>
        moment(a[keyToCheck]).valueOf() - moment(b[keyToCheck]).valueOf()
    );

  public descendingDateSort = (
    arr: Array<any>,
    keyToCheck: string
  ): Array<any> =>
    [...arr].sort(
      (a, b) => moment(b[keyToCheck]).valueOf() - moment(a).valueOf()
    );

  public isSameOrAfter = (date1: string, date2 = moment()): boolean =>
    moment(date1).isSameOrAfter(date2);

  public isAfter = (date1: string, date2 = moment()): boolean =>
    moment(date1).isAfter(date2);

  public isBefore = (date1: string, date2 = moment()): boolean =>
