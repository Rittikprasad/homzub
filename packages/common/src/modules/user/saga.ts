...
export function* login(action: IFluxStandardAction<ILoginPayload>) {
  ...
  let trackData: IAuthenticationEvent = {
    source: is_referral
      ? REFERRAL
      : data.action === LoginTypes.EMAIL
      ? EMAIL
      : OTP,
    ...(data.action === LoginTypes.EMAIL && { email: data.payload.email }),
    ...(data.action === LoginTypes.OTP && {
  ...
  try {
    ...
    yield call(StorageService.set, StorageKeys.USER, tokens);
    ...
    const handleCallback = (): void => {
      if (is_referral || is_from_signup) {
        AnalyticsService.track(EventType.SignupSuccess, {
          ...trackData,
          email: userData.email,
        });
      } else {
        AnalyticsService.track(EventType.LoginSuccess, {
          ...trackData,
          email: userData.email,
        });
      }
    ...
    yield call(AnalyticsService.setUser, userData, handleCallback);
    ...
    if (is_referral || is_from_signup) {
      yield call(AnalyticsService.track, EventType.SignupFailure, trackData);
    } else {
      yield call(AnalyticsService.track, EventType.LoginFailure, trackData);
    }

    yield call(AlertHelper.error, {
      message: error,
...
export function* deactivateUserAccount(
  action: IFluxStandardAction<IAuthCallback>
) {
  ...
  try {
    ...
    if (currentLanguage !== response.languageCode) {
      yield StorageService.set(
        StorageKeys.USER_SELECTED_LANGUAGE,
        response.languageCode
      );
      yield I18nService.changeLanguage(
        response.languageCode as SupportedLanguages
      );
    }
  } catch (e) {
    const error = ErrorUtils.getErrorMessage(e.details);
...
export function* updateUserPreferences(
  action: IFluxStandardAction<IUpdateUserPreferences>
) {
  const { payload } = action;
  try {
    const response: UserPreferences = yield call(
    ...
    if (
      payload &&
      UserPreferencesKeys.LanguageKey === Object.keys(payload)[0]
    ) {
      yield StorageService.set(
        StorageKeys.USER_SELECTED_LANGUAGE,
        response.languageCode
...
export function* getBankInfo(action: IFluxStandardAction<number>) {
  try {
    const response = yield call(
      UserRepository.getUserBankInfo,
...
export function* watchUser() {
  ...
  yield takeEvery(
    UserActionTypes.AUTH.DEACTIVATE_USER_ACCOUNT,
  ...
  yield takeEvery(
    UserActionTypes.UPDATE.USER_PREFERENCES,
    updateUserPreferences
  );
  yield takeEvery(UserActionTypes.GET.USER_ASSETS, getUserAssets);
  yield takeEvery(
    UserActionTypes.GET.FAVOURITE_PROPERTIES,
  ...
  yield takeEvery(
    UserActionTypes.GET.USER_COIN_TRANSACTION,
    getUserTransaction
  );
